# Implementation Plan: LexLight v1

## Overview
Build LexLight per [SPEC.md](SPEC.md): a zero-account web tool that analyzes pasted/uploaded legal documents (Employment, NDA, Residential Lease) via a 3-pass Claude pipeline, returning a structured plain-English risk analysis with jurisdiction-aware mismatch detection, missing-clause checks, key dates, rights, obligations, follow-up Q&A, and export.
...
- [ ] `app/page.tsx` renders a placeholder "LexLight" heading
...
**Description:** Implement `lib/userId.ts` — a UUID v4 generated on first visit, persisted in `localStorage` (key `lexlight_uid`) with `indexedDB` fallback, mirrored to both. Pure client module. No network calls.
...
**Acceptance criteria:**
- [ ] Renders modal on first mount when `sessionStorage.lexlight_disclaimer !== 'ack'`
...
- [ ] Triggers download as `lexlight-analysis-{YYYY-MM-DD}.md`
...
- [ ] Footer: page numbers + "Generated by LexLight — not legal advice"
- [ ] File name `lexlight-analysis-{YYYY-MM-DD}.pdf`
- [ ] No embedded raw document text — only the analysis

**Verification:**
- [ ] Unit test: PDF Blob produced, byte length > 0, contains "LexLight" string via pdf-parse
